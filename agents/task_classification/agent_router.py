...
class AgentRouter:
    ...
    async def route_to_appointment(self, task: str) -> AsyncGenerator[str, None]:
        ...
        # 生成思考提示
        yield "[THOUGHT][调度中心] 调度中心：识别为工单调度任务，转派单引擎处理。"
    ...
    async def route_to_consultation(self, task: str) -> AsyncGenerator[str, None]:
        ...
        # 生成思考提示
        yield "[THOUGHT][调度中心] 调度中心：识别为知识咨询任务，转知识库自服务引擎处理。"
    ...
    async def handle_unsupported_task(self, category: str) -> AsyncGenerator[str, None]:
        ...
        Yields:
            str: 回复内容
        """
        reply = "暂不支持该类型任务。请只提交和IT运维、工单调度相关的问题。"
        yield "[REPLY][调度中心]"
        for char in reply:
            yield char
    ...
    def get_available_services(self) -> list:
        ...
        if self.appointment_agent:
            services.append("工单调度服务")
        if self.consultant_agent:
            services.append("知识库服务")
        return services
