...
class TechnicianFinder:
    ...
    def find_specific_technician(self, technician_name: str, start_time: datetime, 
        ...
        if yield_func:
            yield_func(f"[THOUGHT][派单引擎] 用户指定了工程师：{technician_name}，正在查询信息...\n")
        
        specific_tech = appointment_service.get_technician_by_name(technician_name)
        if specific_tech:
            if yield_func:
                yield_func(f"[THOUGHT][派单引擎] 找到工程师：{specific_tech['name']}，正在检查负载...\n")
            
            if appointment_service.is_technician_available(specific_tech["id"], start_time, end_time):
                if yield_func:
                    yield_func(f"[THOUGHT][派单引擎] {technician_name}工程师在指定时间有空\n")
                return specific_tech
            else:
                if yield_func:
                    yield_func(f"[THOUGHT][派单引擎] {technician_name}工程师在指定时间不空闲\n")
                return None
        else:
            if yield_func:
                yield_func(f"[THOUGHT][派单引擎] 未找到名为'{technician_name}'的工程师\n")
            return None
    ...
    def find_similar_available_technician(self, target_technician: Dict[str, Any], 
        ...
        """根据目标工程师的专长查找相似且可用的工程师"""
        # 通过Services层访问数据库
        from services.appointment_service import AppointmentService
        appointment_service = AppointmentService()
        
        if yield_func:
            yield_func(f"[THOUGHT][派单引擎] 正在根据{target_technician['name']}的专长查找相似工程师...\n")
        ...
        if yield_func:
            yield_func(f"[THOUGHT][派单引擎] 根据专长相似度排序，准备检查可用性...\n")
        ...
        for index in indices:
            ...
            if appointment_service.is_technician_available(similar_tech["id"], start_time, end_time):
                if yield_func:
                    yield_func(f"[THOUGHT][派单引擎] 找到相似且可用的工程师：{similar_tech['name']}\n")
                return similar_tech
        
        if yield_func:
            yield_func(f"[THOUGHT][派单引擎] 没有找到相似且可用的工程师\n")
        return None
    ...
    def filter_technicians_by_gender(self, all_techs: list, gender: str) -> list:
        ...
        if gender in ["男", "男性", "男工程师", "male"]:
            target_gender = "男"
        elif gender in ["女", "女性", "女工程师", "female"]:
            target_gender = "女"
        else:
            return all_techs
        ...
        return filtered_techs if filtered_techs else all_techs  # 如果没有匹配的，返回所有工程师
    ...
    def find_available_technician(self, filtered_techs: list, all_techs: list, 
        ...
        if yield_func:
            yield_func("[THOUGHT][派单引擎] 没有找到空闲工程师\n")
        return None
    ...
    def find_technician_with_thought(self, appointment_history: Dict[str, Any], 
        ...
        if not start_time or not end_time:
            if yield_func:
                yield_func("[THOUGHT][派单引擎] 预约时间或时长信息不完整，无法检索工程师\n")
            return None

        if yield_func:
            yield_func("[THOUGHT][派单引擎] 正在解析预约时间和时长...\n")
        ...
        if technician_name and technician_name != "未知":
            ...
            # 如果没有找到目标工程师或相似工程师，返回None
            return None

        # 通用查询逻辑
        if yield_func:
            yield_func("[THOUGHT][派单引擎] 正在检索所有工程师数据...\n")
        
        all_techs = appointment_service.get_all_technicians()
        if not all_techs:
            if yield_func:
                yield_func("[THOUGHT][派单引擎] 没有找到任何工程师数据\n")
            return None

        # 先根据性别筛选工程师
        gender_filtered_techs = self.filter_technicians_by_gender(all_techs, gender)
        if yield_func and gender and gender != "未知":
            yield_func(f"[THOUGHT][派单引擎] 根据性别'{gender}'筛选工程师，找到{len(gender_filtered_techs)}位工程师\n")
        ...
        if yield_func and preference and preference != "无":
            yield_func(f"[THOUGHT][派单引擎] 根据偏好'{preference}'进一步筛选，找到{len(filtered_techs)}位工程师\n")
