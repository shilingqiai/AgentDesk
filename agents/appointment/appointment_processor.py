...
class WeatherMCPTool(BaseTool):
    """环境信息工具"""
    name: str = "get_environment_info"
    description: str = "获取当前环境状态信息"
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(self._base_url, params=params) as response:
                    if response.status == 200:
                        ...
                        return f"当前环境：{description}，温度{temp}°C（体感{feels_like}°C），湿度{humidity}%，风速{wind_speed}m/s"
                    else:
                        return "当前SLA响应时限为2小时，请及时处理工单"
        except Exception as e:
            return "请关注工单SLA时效，确保在响应时限内处理"
...
class AppointmentProcessor:
    ...
    def __init__(self, input_parser: InputParser, technician_finder: TechnicianFinder,
        ...
        if self.llm:
            ...
            self.agent_prompt = ChatPromptTemplate.from_messages([
                ("system", "你是一个智能工单调度助手，可以获取环境信息并生成专业的工单派发成功提示。"),
                ("human", "{input}"),
    ...
    async def handle_unrelated_request(self, user_input: str, unrelated_callback, state) -> AsyncGenerator[str, None]:
        ...
        if unrelated_callback:
            try:
                yield "[REPLY][派单引擎]该问题与工单调度无关，已转调度中心重新分类处理\n"
    ...
    async def handle_complete_appointment(self, appointment_history: Dict[str, Any], 
        ...
        if tech:
            ...
            if tech.get('requires_confirmation'):
                ...
                yield f"[REPLY][派单引擎]{recommendation_msg}"
    ...
    async def _process_successful_appointment(self, tech: Dict[str, Any],
        ...
        if success:
            ...
            # 使用 LLM agent 生成工单派发成功提示
            if self.llm and hasattr(self, 'agent_executor'):
                prompt = f"请获取当前环境信息，然后为用户生成一段专业的工单派发成功提示。工程师：{tech['name']}，技能领域：{tech.get('strength', 'IT运维')}。请提醒用户关注SLA响应时限（2小时响应，8小时解决）。"
                try:
                    result = await self.agent_executor.ainvoke({"input": prompt})
                    agent_output = result.get("output", "")
                    return f"\n系统：工单已派发工程师：{tech['name']}（{tech.get('strength', 'IT运维')}）。派单成功！\n{agent_output}\n"
                except Exception as e:
                    print(f"Agent调用失败: {e}")
    ...
    async def handle_incomplete_info(self, data: Dict[str, Any], appointment_history: Dict[str, Any]) -> AsyncGenerator[str, None]:
        ...
        yield f"[THOUGHT][派单引擎]工单信息不完整，缺少：{', '.join(missing)}，需要询问用户补充信息"
        yield f"[REPLY][派单引擎]{reply}"
