...
class TestAppointmentAgentCoreFeatures:
    ...
    def test_should_extract_user_info_from_natural_language(self):
        ...
        user_input = "我想预约明天下午2点的IT支持，女工程师"
        ...
        # 验证解析结果包含预期信息
        assert result["project"] == "IT支持", f"应该提取到IT支持项目，但得到：{result['project']}"
        assert result["gender"] == "女", f"应该提取到女工程师偏好，但得到：{result['gender']}"
    ...
    def test_should_track_appointment_state_correctly(self):
        ...
        用户分步骤提供信息：
        1. "我要预约IT支持" -> 应该记录项目=IT支持，其他为空
        2. "明天下午2点" -> 应该记录时间，保持项目=IT支持
        3. "女工程师" -> 应该记录性别偏好，保持之前信息
        """
        agent = AppointmentAgent()
        ...
        assert agent.appointment_history["project"] == "IT支持"
        ...
        assert agent.appointment_history["project"] == "IT支持"  # 应该保持
...
class TestAppointmentAgentEdgeCases:
    ...
    def test_should_reset_state_properly(self):
        ...
        # 设置一些状态
        agent.appointment_history["project"] = "IT支持"
