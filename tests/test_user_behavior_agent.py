...
class TestUserBehaviorAgentCoreFeatures:
    ...
    def test_should_record_user_behavior_correctly(self):
        ...
        try:
            ...
            assert latest_behavior["service_type"] == "IT支持"
    ...
    def test_should_analyze_user_preferences_correctly(self):
        ...
        behaviors = [
            {"user_id": user_id, "action": "appointment", "service_type": "IT支持", "preferred_time": "下午"},
            {"user_id": user_id, "action": "appointment", "service_type": "IT支持", "preferred_time": "下午"},
            {"user_id": user_id, "action": "appointment", "service_type": "运维", "preferred_time": "上午"},
            {"user_id": user_id, "action": "consultation", "topic": "IT支持效果"},
        ]
        ...
        try:
            ...
            # 应该识别出IT支持是最常选择的服务（这个测试可能会失败）
            preferred_service = preferences.get("preferred_service_type")
            assert preferred_service == "IT支持", f"应该识别IT支持为偏好服务，但得到：{preferred_service}"
    ...
    def test_should_generate_personalized_recommendations(self):
        ...
        historical_behaviors = [
            {"user_id": user_id, "service_type": "深层IT支持", "satisfaction_rating": 5},
            {"user_id": user_id, "service_type": "肩颈IT支持", "satisfaction_rating": 4},
        ...
        try:
            ...
            massage_related = any("IT支持" in service for service in recommended_services)
            assert massage_related, f"推荐应该包含IT支持相关服务，但得到：{recommended_services}"
    ...
    def test_should_identify_behavior_patterns(self):
        ...
        for i in range(10):
            patterns_data.append({
                ...
                "service_type": "IT支持" if i % 2 == 0 else "运维"
            })
    ...
    def test_should_provide_behavior_insights(self):
        ...
        # 创建有趣的行为数据用于洞察
        insight_data = [
            {"user_id": user_id, "action": "consultation", "topic": "IT支持效果", "timestamp": datetime.now()},
            {"user_id": user_id, "action": "appointment", "service_type": "IT支持", "timestamp": datetime.now()},
...
class TestUserBehaviorAgentDataManagement:
    ...
    def test_should_manage_user_preferences_storage(self):
        ...
        # 设置用户偏好
        preferences = {
            "preferred_service_types": ["IT支持", "运维"],
        ...
        try:
            ...
            assert "IT支持" in retrieved_prefs["preferred_service_types"]
    ...
    def test_should_handle_data_aggregation(self):
        ...
        # 创建多个用户的数据
        users_data = [
            {"user_id": "user_1", "service_type": "IT支持", "satisfaction": 5},
            {"user_id": "user_2", "service_type": "IT支持", "satisfaction": 4},
            {"user_id": "user_3", "service_type": "运维", "satisfaction": 3},
            {"user_id": "user_1", "service_type": "IT支持", "satisfaction": 5},
        ]
        ...
        try:
            ...
            assert "IT支持" in str(service_stats), "聚合结果应该包含IT支持统计"
            assert "运维" in str(service_stats), "聚合结果应该包含运维统计"
